...
public class DeployExample extends AbstractVerticle {
  ...
  public void start() throws Exception {
    ...
    vertx.deployVerticle("io.vertx.example.web.auth.Server");

    // Deploy another instance and  want for it to start
    vertx.deployVerticle("io.vertx.example.web.auth.Server", res -> {
      if (res.succeeded()) {
    ...
    vertx.deployVerticle("io.vertx.example.web.auth.Server", new DeploymentOptions().setConfig(config));

    // Deploy 10 instances
    vertx.deployVerticle("io.vertx.example.web.auth.Server", new DeploymentOptions().setInstances(10));

    // Deploy it as a worker verticle
    vertx.deployVerticle("io.vertx.example.web.auth.Server", new DeploymentOptions().setWorker(true));
